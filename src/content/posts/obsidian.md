...
image: /public/assets/images/obsidian.webp
tags:
  - Obsidian
  - Markdown
draft: false
...
![](/public/assets/images/obsidian-1.webp)
然后点击 **管理仓库** ，然后根据你所需要的情况进行选择
![](/public/assets/images/obsidian-2.webp)
...
首先，确保你将 `src/content` 作为仓库根目录，因为 `src/content/posts` 存放博客文章，而 `src/content/spec` 存放关于等特殊MarkDown页面，他们都可能需要依赖图片，所以建议将仓库设置在他们的上一级文件夹。我们的图片将存放在 `src/content/public/assets/images` 在 `posts` 或 `spec` 的相对路径引用格式则为 `/public/assets/images/xxx.webp` （不用担心，黑曜石会自动管理，你无需手打）
点击 **设置** 
![](/public/assets/images/obsidian-3.webp)
如图配置，这样我们就解决了第一个问题
![](/public/assets/images/obsidian-4.webp)
...
![](/public/assets/images/obsidian-5.webp)
接下来，尝试使用任一截图工具，如 **QQ截图** ，截图后使用 **Ctrl+V** 粘贴进文章，你应当能看到类似这样的图片链接了
```bash
![](/public/assets/images/obsidian.webp)
```
# 黑曜石如何强大？
`published` 字段可以通过点点点实现
![](/public/assets/images/obsidian-6.webp)
通用字段可以直接填充曾经写过的
![](/public/assets/images/obsidian-7.webp)
`tags` 字段只需要你专注于标签，无需再手动管理格式
![](/public/assets/images/obsidian-8.webp)
布尔字段通过勾选来处理 `true` 和 `false`
![](/public/assets/images/obsidian-9.webp)